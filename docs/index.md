...
Before we can look into these applications, first we need to know if it is even possible to correctly identify the current driver of a car. To test this, Girma, Yan and Homaifar have described a Deep LSTM based method of identifying drivers based on their driving style in [their paper](https://ieeexplore.ieee.org/abstract/document/8995202). In this blog, we test their model by building our own implementation of it from scratch. We will take a look at the implementation they used, and see if it is as robust as they have claimed in the paper by performing comparable experiments to them. 

## The Data
Modern cars are all equipped with an On-Board Diagnostics (OBD) connector. OBD is a standard that has been implemented to make most diagnostic information available without the need for manufacturer-specific hardware to have access to it. Via this connection, one can directly read most sensor output from the car. Which sensors are available can still depend on the make and model of the car, but most cars have a set of standard sensors. The paper uses two different datasets to test their model. One of these, the dataset used to test the model in this blog, is a set of 51 sensors, including, but not limited to: steering wheel input, engine RPM, throttle input, vehicle speed and fuel consumption. For a full list of sensors used, please see [Appendix A](#a-list-of-features) at the bottom of the page. 

The data used in this research was created by Il Kwak, Woo and Kang Kim [(Know Your Master: Driver Profiling-based Anti-theft Method, 2016)](https://arxiv.org/abs/1704.05223), and was generated by having ten different people make a round trip, and recording the sensor data from these drives. The data was then labelled based on the driver. This results in a set of time-series data containing timesteps with 51 features each. 

## Why an LSTM?
A Long Short-Term Memory network, or LSTM for short, is a specialized form of neural network. Like convolutional networks are highly specialised for 2D image data, an LSTM is highly specialised for time series data. 

Since the sensor data is all collected with a set time interval, one sample after the other holds some interdependence and is order dependent. To illustrate the interdependence of the samples, consider the following example: When taking two consecutive samples, a car driving 10 km/h for the first sample and 0 km/h for the second sample probably came to a slow stop, while a car driving 50 km/h for the first sample and 0 km/h for the second sample had to do an emergency stop. The order for these samples then also influences the interpretation, as a car going from 50 km/h to 0 km/h is braking, while a car going from 0 km/h to 50 km/h is accelerating.
